...
export default class Comment extends React.Component {
    ...
    getHowLongAgo(time) {
        ...
        if (secondsElapsed < 60) return Math.round(secondsElapsed) + " seconds";
        if (secondsElapsed < 3600) return Math.round(secondsElapsed/3600) + " minutes";
        const hoursElapsed = secondsElapsed/3600;
        if (hoursElapsed < 72) return Math.round(hoursElapsed) + " hours";
        return Math.round(hoursElapsed/24) + " days";
    }
    ...
    render() {
        ...
        return (
            <div className="comment">
                <img className="profile-picture" height="36px" width="auto" src={this.getProfilePicture(content.picture)} alt="Profile picture"/>
